    ...
    <AdminLayout title="Dashboard">
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ tvShow ? tvShow.name : '' }} - {{ season ? season.name : '' }} Episodes Index
            </h2>
        </template>
                        ...
                        <div class="w-full overflow-x-auto">
                            <Table>
                                ...
                                <TableRow v-for="episode in episodes.data" :key="episode.id">
                                    ...
                                    <TableData style=" display: flex; justify-content: space-between">
                                        <ButtonLink
                                                :link="route('admin.episodes.edit',[tvShow.id,season.id,episode.id])">
                                            Edit
                                        </ButtonLink>
                                        <ButtonLink class="bg-red-500 hover:bg-red-700" method="delete" as="button"
                                                    type="button"
                                                    :link="route('admin.episodes.destroy',[tvShow.id,season.id,episode.id])">
                                            Delete
                                        </ButtonLink>
                                    </TableData>
                                </TableRow>
...
<script setup>
    ...
    function generateEpisode() {
        router.post(`/admin/tv-shows/${props.tvShow.id}/seasons/${props.season.id}/episodes`, {episodeNumber: episodeNumber.value}, {
